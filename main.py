...
from modules.calculator import calculate_nets
...
def calculate_lgs_countdown():
    ...
    return f"{diff.days} Gün {hours} Saat"
...
def main():
    ...
    while True:
        ...
        header_text = (
            ...
            f"[bold white]⏳ Sınava Kalan Süre:[/bold white] {calculate_lgs_countdown()}"
        )
        ...
        menu = (
            ...
            "  [bold cyan]\[3][/bold cyan] ⚡ Hızlı Deneme Net Hesaplayıcı\n"
            "  [bold cyan]\[4][/bold cyan] 🔗 Faydalı LGS Linkleri\n"
            "  [bold cyan]\[5][/bold cyan] 🛠️ Öğretmen / Admin Paneli\n"
            "  [bold cyan]\[6][/bold cyan] 📢 Sınıf Duyuruları\n"
            "  [bold red]\[0][/bold red] 🚪 Çıkış\n"
        )
        console.print(menu)
        
        choice = Prompt.ask("[bold yellow]Seçiminiz (0-6)[/bold yellow]", choices=["0", "1", "2", "3", "4", "5", "6"])
